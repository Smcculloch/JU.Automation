...
public class ActionStep4CreateRules : ActionStepBase<ActionStep4CreateRules, SunriseModel>
{
    ...
    public override async Task<SunriseModel> ExecuteStep(SunriseModel model)
    {
        ...
        model.Rules.Trigger = await CreateTriggerRule(model.Index, model.Group, model.TriggerSensor, model.Scenes.Init,
        ...
        model.Rules.TurnOff = await CreateTurnOffRule(model.Index, model.TriggerSensor, model.Schedules.TurnOff,
            model.WakeupTime, model.DepartureTime);

        return model;
    }

    private async Task<Rule> CreateTriggerRule(int index, Group group, Sensor triggerSensor, Scene initScene,
        Schedule transitionUpSchedule)
    {
        var sunriseTriggerRule = new Rule
        {
            Name = $"{Constants.Automation.Sunrise}{index}{Constants.Entity.Rule}{Constants.Stage.Start}",
    ...
    private async Task<Rule> CreateTurnOffRule(int index, Sensor triggerSensor, Schedule turnOffSchedule,
        TimeSpan wakeupTime, TimeSpan departureTime)
    {
        var turnOffTime = departureTime - wakeupTime;

        var sunriseTurnOffRule = new Rule
        {
            Name = $"{Constants.Automation.Sunrise}{index}{Constants.Entity.Rule}{Constants.Stage.TurnOff}",
            Conditions = new List<RuleCondition>
            {
                new()
                {
