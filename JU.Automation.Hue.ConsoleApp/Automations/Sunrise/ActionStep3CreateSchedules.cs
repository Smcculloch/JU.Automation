...
public class ActionStep3CreateSchedules : ActionStepBase<ActionStep3CreateSchedules, SunriseModel>
{
    ...
    public override async Task<SunriseModel> ExecuteStep(SunriseModel model)
    {
        ...
        model.Schedules.Start =
            await CreateStartSchedule(model.Index, model.TriggerSensor, model.RecurringDay, model.WakeupTime);
        ...
        TimeSpan startTime)
    {
        ...
        {
            Name = $"{Constants.Automation.Sunrise}{index}{Constants.Entity.Schedule}{Constants.Stage.Start}",
    ...
    private async Task<Schedule> CreateTransitionUpSchedule(int index, Scene transitionUpScene)
    {
        ...
        {
            Name = $"{Constants.Automation.Sunrise}{index}{Constants.Entity.Schedule}{Constants.Stage.TransitionUp}",
            Command = new InternalBridgeCommand
            {
        ...
        Console.WriteLine(
            $"Schedule ({sunriseTransitionUpSchedule.Name}) with id {sunriseTransitionUpScheduleId} created");
    ...
    private async Task<Schedule> CreateTurnOffSchedule(int index, Scene turnOffScene)
    {
        ...
        {
            Name = $"{Constants.Automation.Sunrise}{index}{Constants.Entity.Schedule}{Constants.Stage.TurnOff}",
            Command = new InternalBridgeCommand
            {
                Address = $"/api/{_settingsProvider.AppKey}/groups/0/action",
                Body = new SceneCommand
