...
public class AutomationSetupActionStep3CreateSchedulesTests
{
        ...
        string wakeupTurnOffScheduleId)
    {
        ...
        var nameTransitionUp = $"{Constants.Automation.Wakeup}1{Constants.Entity.Schedule}{Constants.Stage.TransitionUp}";
        var nameTransitionDown = $"{Constants.Automation.Wakeup}1{Constants.Entity.Schedule}{Constants.Stage.TransitionDown}";
        var nameTurnOff = $"{Constants.Automation.Wakeup}1{Constants.Entity.Schedule}{Constants.Stage.TurnOff}";

        _hueClient.Setup(m => m.CreateScheduleAsync(It.Is<Schedule>(schedule =>
                      schedule.Name == nameTransitionUp && schedule.LocalTime.TimerTime == timerTime)))
        ...
        _hueClient.Setup(m => m.CreateScheduleAsync(It.Is<Schedule>(schedule =>
                      schedule.Name == nameTransitionDown && schedule.LocalTime.TimerTime == timerTime)))
        ...
        _hueClient.Setup(m => m.CreateScheduleAsync(It.Is<Schedule>(schedule =>
                      schedule.Name == nameTurnOff && schedule.LocalTime.TimerTime == timerTime)))
